  ...
  return (
    <div style={{ border:`1px solid rgba(255,255,255,0.07)`, borderRadius:10, overflow:'hidden', background:'rgba(0,0,0,0.5)' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0.5rem 1rem', borderBottom:`1px solid rgba(255,255,255,0.06)`, background:'rgba(255,255,255,0.02)' }}>
        <span style={{ fontSize:10, fontFamily:"'DM Mono',monospace", color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.08em' }}>{lang}</span>
...
export default function EndpointViewer() {
  ...
  const [expandedEnum, setExpandedEnum] = useState(null)  // fieldName of expanded enum
      ...
      <main style={{ maxWidth:900, margin:'0 auto', padding:'2rem clamp(1.5rem,5vw,3rem)', display:'flex', flexDirection:'column', gap:20 }}>
                ...
                <div key={i} style={{ border:`1px solid ${C.border}`, borderRadius:10, overflow:'hidden' }}>
                  <div style={{ display:'flex', alignItems:'center' }}>
                    <button onClick={()=>setExpanded(isOpen?null:i)} style={{ flex:1, minWidth:0, display:'flex', alignItems:'center', gap:12, padding:'0.75rem 1rem', background:'transparent', border:'none', cursor:'pointer', textAlign:'left', transition:'background 150ms', color:C.fg }} onMouseEnter={e=>e.currentTarget.style.background='rgba(255,255,255,0.02)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                      ...
                      <span style={{ fontSize:11, color:'#94A3B8', flexShrink:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:240, display:'none' }}>{ep.description}</span>
                    </button>
                    <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0, paddingRight:'1rem' }}>
                      <button onClick={()=>copyUrl(ep.url,i)} style={{ fontSize:11, padding:'0.2rem 0.6rem', borderRadius:6, border:`1px solid ${C.border}`, background:'transparent', color:urlCopied===i?C.accent:C.muted, cursor:'pointer', transition:'color 150ms', fontFamily:"'DM Sans',sans-serif" }}>
                        {urlCopied===i?'✓':'⎘'}
                      </button>
                      <span aria-hidden="true" style={{ color:'#94A3B8', fontSize:11, transition:'transform 200ms', display:'inline-block', transform:isOpen?'rotate(180deg)':'rotate(0deg)' }}>▾</span>
                    </div>
                  </div>
                  ...
                  {isOpen && (
                    <div style={{ borderTop:`1px solid rgba(255,255,255,0.06)`, padding:'1rem', background:'rgba(0,0,0,0.2)', display:'flex', flexDirection:'column', gap:14 }}>
                      <p style={{ fontSize:12, color:'#94A3B8' }}>{ep.description}</p>

                      {ep.method==='GET' && !ep.hasId && (
                        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(100px,1fr))', gap:8 }}>
                          {['?page=1','?limit=10','?sort=-createdAt','?filter=name:john'].map(p=>(
                            <span key={p} style={{ fontFamily:"'DM Mono',monospace", fontSize:10, padding:'0.35rem 0.6rem', borderRadius:6, background:'rgba(255,255,255,0.03)', border:`1px solid rgba(255,255,255,0.06)`, color:'#94A3B8', textAlign:'center', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p}</span>
                          ))}
                        </div>
                      )}
        ...
        {!loading && fields.length>0 && (
          <div style={{ border:`1px solid ${C.border}`, borderRadius:10, overflow:'hidden' }}>
            ...
            <div>
              {fields.map((f,i)=>(
                <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:12, padding:'0.6rem 1rem', borderBottom:i<fields.length-1?`1px solid rgba(255,255,255,0.04)`:'none' }}>
                  <span style={{ fontFamily:"'DM Mono',monospace", fontSize:13, color:'rgba(255,255,255,0.7)', width:144, flexShrink:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{f.fieldName}</span>
                  <span style={{ fontSize:11, padding:'0.15rem 0.5rem', borderRadius:6, border:'1px solid rgba(34,197,94,0.2)', background:'rgba(34,197,94,0.08)', color:C.accent, fontFamily:"'DM Mono',monospace", flexShrink:0 }}>{f.type}</span>
                  {f.required && <span style={{ fontSize:10, color:'rgba(255,255,255,0.5)', flexShrink:0, paddingTop:2 }}>required</span>}
                  {f.type==='enum' && f.values?.length>0 && <span onClick={() => setExpandedEnum(expandedEnum === f.fieldName ? null : f.fieldName)} style={{ fontSize:10, fontFamily:"'DM Mono',monospace", color:`${C.yellow}99`, flex:1, minWidth:0, ...(expandedEnum === f.fieldName ? { wordBreak:'break-word', cursor:'pointer' } : { overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', cursor:'pointer' }), paddingTop:2 }} title={expandedEnum === f.fieldName ? 'Tap to collapse' : 'Tap to expand'}>[{f.values.join(', ')}]</span>}
                </div>
              ))}
            </div>
          </div>
