    ...
    <div style={{ border:`1px solid ${C.border}`, borderRadius:12, overflow:'hidden', background:'rgba(0,0,0,0.35)', backdropFilter:'blur(8px)' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0.75rem 1rem', borderBottom:`1px solid ${C.border}`, background:'rgba(255,255,255,0.02)', flexWrap:'wrap', gap:8 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <span style={{ fontSize:10, fontFamily:"'DM Mono',monospace", color:'#94A3B8', background:'rgba(255,255,255,0.05)', padding:'0.15rem 0.5rem', borderRadius:5 }}>last 100</span>
          {logs.length > 0 && <span style={{ fontSize:10, color:'#94A3B8' }}>{logs.length} entries</span>}
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <button
            onClick={() => setAutoRefresh(p => !p)}
            style={{ display:'flex', alignItems:'center', gap:6, fontSize:11, padding:'0.3rem 0.75rem', borderRadius:6, border:autoRefresh?`1px solid rgba(34,197,94,0.4)`:`1px solid ${C.border}`, background:autoRefresh?'rgba(34,197,94,0.1)':'transparent', color:autoRefresh?C.accent:'#94A3B8', cursor:'pointer', transition:'all 150ms', fontFamily:"'DM Sans',sans-serif" }}
          >
          ...
          <button onClick={fetchLogs} style={{ fontSize:13, color:'#94A3B8', background:'transparent', border:'none', cursor:'pointer', padding:'0.3rem 0.5rem', borderRadius:6, transition:'color 150ms' }} onMouseEnter={e=>e.currentTarget.style.color='rgba(255,255,255,0.7)'} onMouseLeave={e=>e.currentTarget.style.color='#94A3B8'}>↻</button>
          {logs.length > 0 && <button onClick={() => setLogs([])} style={{ fontSize:11, color:'#94A3B8', background:'transparent', border:'none', cursor:'pointer', padding:'0.3rem 0.5rem', borderRadius:6, transition:'color 150ms', fontFamily:"'DM Sans',sans-serif" }} onMouseEnter={e=>e.currentTarget.style.color='rgba(239,68,68,0.7)'} onMouseLeave={e=>e.currentTarget.style.color='#94A3B8'}>Clear</button>}
        </div>
      </div>

      {/* Log rows */}
      <div tabIndex={0} role="region" aria-label="Request log entries" style={{ maxHeight:288, overflowY:'auto' }}>
        {loading ? (
          Array.from({ length: 5 }).map((_, i) => <LogRowSkeleton key={i}/>)
        ) : logs.length === 0 ? (
          <div style={{ padding:'3rem 1rem', textAlign:'center' }}>
            <p style={{ fontSize:12, color:'#94A3B8' }}>No requests recorded yet</p>
            <p style={{ fontSize:11, color:'#94A3B8', marginTop:4 }}>Hit your API endpoints to see logs appear here</p>
          </div>
        ) : (
          logs.map(log => {
            const mc = METHOD_COLOR[log.method] || { text:'#94A3B8', bg:'rgba(255,255,255,0.05)', border:'rgba(255,255,255,0.1)' }
            return (
              <div key={log._id} style={{ display:'flex', alignItems:'center', gap:10, padding:'0.5rem 1rem', fontFamily:"'DM Mono',monospace", fontSize:11, borderBottom:`1px solid rgba(255,255,255,0.04)`, transition:'background 150ms', cursor:'default' }} onMouseEnter={e=>e.currentTarget.style.background='rgba(255,255,255,0.02)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                ...
                <span style={{ flex:1, color:'#94A3B8', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{log.path}</span>
                <span style={{ flexShrink:0, textAlign:'right', width:52, color:log.duration>1000?C.yellow:'#94A3B8' }}>{log.duration}ms</span>
                <span style={{ flexShrink:0, color:'#94A3B8', width:56, textAlign:'right' }} key={tick}>{timeAgo(log.timestamp)}</span>
              </div>
            )
          })
        )}
      ...
      {logs.length > 0 && (
        <div style={{ display:'flex', alignItems:'center', gap:16, padding:'0.5rem 1rem', borderTop:`1px solid rgba(255,255,255,0.06)`, background:'rgba(255,255,255,0.01)' }}>
          <span style={{ fontSize:10, color:'#94A3B8', fontFamily:"'DM Mono',monospace" }}>avg <span style={{color:'#94A3B8'}}>{avgMs}ms</span></span>
          <span style={{ fontSize:10, color:'#94A3B8', fontFamily:"'DM Mono',monospace" }}>errors <span style={{color:errorPct>0?'rgba(239,68,68,0.7)':'#94A3B8'}}>{errorPct}%</span></span>
          <span style={{ fontSize:10, color:'#94A3B8', marginLeft:'auto', fontFamily:"'DM Mono',monospace" }}>{autoRefresh?'refreshes every 5s':'auto-refresh off'}</span>
        </div>
      )}
